...
class InterfaceRepository(Generic[T]):
    ...
    def find_by_id(self, id_ : str) -> dict:
        current_collection = self.data_base[self.collection]
        _id = ObjectId(id_)
        # if the _id not exist, the document= None
    ...
    def update(self, id_ : str, item : T) -> dict:
        ...
        item = item.__dict__
        update_item = {"$set": item}
        document = current_collection.update_one({':id': _id}, update_item)
        return {"updated_count": document.matched_count}

    def delete(self, id_ : str) -> dict:
        current_collection = self.data_base[self.collection]
        _id = ObjectId(id_)
        result = current_collection.update_one({'_id': _id})
    ...
    def get_values_db_ref(self, document: dict) -> dict:
        for key in document.keys():
            ...
            if isinstance(value, DBRef):
                ...
                document_ref = collection_ref['_id'].__str__()
                document[key] = document_ref
                document[key] = self.get_values_db_ref(document[key])
            elif isinstance(document.get(key), list) and len(value) > 0:
    ...
    def get_values_db_ref_from_list(self, list_: list) -> list:
        ...
        for item in list_:
            ...
            # TODO check if in this point each document_ref needs to call get_values_db_ref
